...
namespace RENDER_NAMESPACE {
    namespace core {
        namespace bsdf {
            ...
            BXDFGlossyDiffuseReflection::BXDFGlossyDiffuseReflection(const Spectrum &Rd, const Spectrum &Rs,
                                                                     const MicrofacetDistribution *dist) :
                    BXDF(BXDFType(BSDF_REFLECTION | BSDF_GLOSSY)),
                    _Rd(Rd), _Rs(Rs), _distribution(dist) {}

            Spectrum BXDFGlossyDiffuseReflection::f(const Vector3F &wo, const Vector3F &wi) const {
            ...
            Spectrum BXDFGlossyDiffuseReflection::sampleF(const Vector3F &wo, Vector3F *wi, Float *pdf,
            ...
            Float BXDFGlossyDiffuseReflection::samplePdf(const Vector3F &wo, const Vector3F &wi) const {
                if (wo.y * wi.y < 0) {
                    return 0;
                }
